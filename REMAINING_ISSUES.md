...
> 生成时间: 2026-04-13
> 状态: 基于 CHANGELOG 和最新代码审查结果整理，按优先级排序
...
- **状态**: 大部分已修复，个别残留
- **问题描述**:
  - 主流业务接口已统一使用 `ApiResponseConst` 和 `MessageConst`
  - `system.py` 健康检查接口仍使用独立的 `{status, timestamp, version}` 格式，与标准响应结构不一致
  - 少数边缘接口的数据嵌套层次仍有差异
- **影响**: 前端需要额外兼容逻辑，增加维护成本
- **建议修复**: 全量扫描 `backend/app/api/routes/` 下所有 `return {` 语句，统一使用常量；健康检查接口可保留独立格式或统一为标准响应
- **相关文件**:
  - `backend/app/api/routes/system.py`
  - `backend/app/api/routes/` 下其他路由文件

### P1-4 签到列表查询缺少分页/限制
- **状态**: 部分修复
- **问题描述**:
  - `get_all_checkins` 已设置默认 `limit=200`，API 层也加了参数校验，但 `get_today_checkins` 仍无 `limit` 参数
  - 教师端和 Admin 端查看签到记录时，学期末 `get_today_checkins` 仍可能返回大量记录
  - 缺少服务端分页组件，前端只能做全量渲染
- **影响**: 前端渲染卡顿，后端内存和带宽压力增加
- **建议修复**:
  - 为 `get_today_checkins` 增加默认 `limit` 参数
  - 教师端/Admin 端签到记录页面增加分页组件（服务端分页）
- **相关文件**:
...
2. **如果要修代码规范**: 优先处理 **P1-3（后端 API 响应格式混用）**
3. **如果要修安全**: 关注 **P0-2（JWT 安全机制）**，用户指定暂缓
