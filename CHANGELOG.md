...
#### 功能
- 座位系统：学生在教室座位图上点座「坐下」+ 输验证码完成签到
- 教室管理（管理员/教师）：按行列布局创建教室（自动生成 `P{row}{col}` 座位编号），支持改名、调整布局、归档
- 学生签到页座位图流程：选座 → 验证码 → 入座动画；无座位图的教室保持原纯验证码流程不变
- 教师端课堂实时座位图：四态展示（空/固定/已坐/我的），支持标记电脑故障座位与课堂内临时调座（下课自动失效）
- 学生「我的座位」查询：当前学期各教室固定座位

#### 前端组件
- `src/components/seats/` 组件族：SeatMap（深色局部作用域，含走廊/门/讲台）、SeatUnit（四态+故障）、StickFigure（六姿势）、入座动画状态机

#### 数据模型
- 新增 `classrooms` / `seats` / `seat_assignments` / `seat_session_overrides` 四张表
- `checkin_records` 新增 `seat_id` 列及同课堂同座位唯一索引

### 变更

#### API
- `POST /api/v1/checkin` 扩展 `seat_id`（启用座位图的课堂学生必填）
- 课堂会话响应新增 `seat_classroom_id`，前端据此分流签到流程
