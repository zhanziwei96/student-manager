...
class TestRateLimitHttpSemantics:
    ...
    def test_checkin_rate_limit_returns_429(self, client, student_user):
        """测试签到接口限流返回 429"""
        # 先登录
        ...
        with patch("app.api.routes.checkin.check_rate_limit", return_value=False):
            response = client.post("/api/v1/checkin", json={
                "student_id": "S001",
                "student_name": "学生1"
            })
            assert response.status_code == 429, f"签到限流应返回 429，实际返回 {response.status_code}"
            data = response.json()
            assert "请求过于频繁" in data.get("message", "")
...
class TestRateLimitConfig:
    ...
    def test_rate_limit_settings_structure(self):
        ...
        assert hasattr(settings.rate_limit, "checkin_max_requests")
        assert hasattr(settings.rate_limit, "checkin_window_seconds")
