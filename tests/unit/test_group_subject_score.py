...
def test_group_has_course_score_fields():
    """小组有课程和分数字段"""
    group = Group(
        class_id=1, semester_id=1, name="第一组", leader_student_id="TEST001",
...
def test_group_score_default_zero():
    """小组分数默认 0"""
    group = Group(class_id=1, semester_id=1, name="第一组", leader_student_id="TEST001")
    assert group.score == 0.0
...
def test_group_score_log_creation():
    ...
    log = GroupScoreLog(
        ...
        reason="课堂表现", operator="张老师", semester_id=1
    )
