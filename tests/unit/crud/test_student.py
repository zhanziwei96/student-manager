...
class TestStudentCRUD:
    ...
    def test_create_student(self, session: Session):
        ...
        assert student.class_name == "一班"
            
    def test_get_student(self, session: Session):
        """测试获取学生"""
        # 先创建学生
        create_student(session, "S002", "李四", "一班")
    ...
    def test_get_students(self, session: Session):
        """测试获取所有学生"""
        create_student(session, "S003", "王五", "一班")
        create_student(session, "S004", "赵六", "二班")
    ...
    def test_get_students_by_class(self, session: Session, seed_refs):
        """测试按班级获取学生"""
        create_student(session, "S005", "张三", "一班")
        create_student(session, "S006", "李四", "二班")
        create_student(session, "S007", "王五", "一班")
        
        students = get_students_by_class(session, "一班")
        assert len(students) == 2
        assert all(s.class_name == "一班" for s in students)
    
    def test_delete_student(self, session: Session):
        """测试删除学生"""
        create_student(session, "S010", "张三", "一班")
    ...
    def test_get_all_classes(self, session: Session, seed_refs):
        """测试获取所有班级"""
        create_student(session, "S011", "张三", "一班")
        create_student(session, "S012", "李四", "二班")
        create_student(session, "S013", "王五", "一班")
    ...
    def test_reset_student_password(self, session: Session):
        ...
        student = create_student(session, "S014", "张三", "一班")
...
class TestStudentPagination:
    ...
    def test_get_students_with_limit_offset(self, session: Session):
        ...
        for i in range(1, 8):
            create_student(session, f"P{i:03d}", f"学生{i}", "一班")
    ...
    def test_get_students_pagination_with_class_filter(self, session: Session, seed_refs):
        ...
        for i in range(1, 6):
            create_student(session, f"P{i:03d}", f"学生{i}", "一班")
        for i in range(6, 9):
            create_student(session, f"P{i:03d}", f"学生{i}", "二班")

        students = get_students(session, class_name="二班", limit=2, offset=1)
    ...
    def test_get_students_by_class_with_limit_offset(self, session: Session, seed_refs):
        ...
        students = get_students_by_class(session, "一班", limit=2, offset=2)
    ...
    def test_get_students_by_classes_with_limit_offset(self, session: Session, seed_refs):
        ...
        create_student(session, "P001", "学生1", "一班")
        create_student(session, "P002", "学生2", "二班")
        create_student(session, "P003", "学生3", "一班")
        create_student(session, "P004", "学生4", "三班")

        students = get_students_by_classes(session, ["一班", "二班"], limit=2, offset=1)
    ...
    def test_count_students_filtered(self, session: Session, seed_refs):
        ...
        assert count_students_filtered(session, class_name="一班") == 3
        assert count_students_filtered(session, class_names=["一班", "二班"]) == 6
    ...
    def test_count_students_filtered_excludes_disabled(self, session: Session):
        ...
        student = create_student(session, "P001", "学生1", "一班")
        create_student(session, "P002", "学生2", "一班")
