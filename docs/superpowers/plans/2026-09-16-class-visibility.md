...
- [x] **Task 6 — 前端：学生端类型与展示适配**：✅ 学生端类型已在 Task 4/5 同步（question.ts class_ids、lostFound.ts class_ids）；列表/详情展示无需改（后端已过滤，class_name 展示串由后端拼装）：学生端问答/失物招领列表无需改（后端已过滤）；检查详情页是否有"可见范围"展示需求（如有则显示可见班级）；类型文件同步
- [x] **Task 7 — 测试与端到端验证**：✅ 后端 814 passed 全绿 + 前端 329 全绿 + vue-tsc 干净；端到端验证 3 项全过：问答可见性（本班见限定+公开/他班只见公开）、失物招领可见性（同上）、评论/认领越权拦截（他班 404）：后端全量 + 前端全量全绿；起服务手工验证：教师发问答勾 2 个班→只有这 2 个班学生能看到；教师发失物招领勾 1 个班→只有该班学生能看到；不勾→所有学生能看到
