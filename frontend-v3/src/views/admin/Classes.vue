...
import { Plus, Users, Search, Loader2, Eye, X, GraduationCap } from 'lucide-vue-next'
  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="text-white/60">管理班级信息和查看班级学生</p>
      </div>
    </div>
    ...
    <!-- Classes grid -->
    <div v-else-if="filteredClasses.length > 0" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <Card
      ...
      >
        <div class="flex items-start justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-white/50">{{ cls.student_count }} 名学生</p>
            </div>
          </div>
        ...
        <div class="mt-4 grid grid-cols-2 gap-4">
          <div class="rounded-lg bg-white/5 p-3">
            <p class="text-xs text-white/50">平均分</p>
            <p class="text-lg font-semibold" :class="getScoreColor(cls.average_score)">
              {{ cls.average_score }}
            </p>
          </div>
          <div class="rounded-lg bg-white/5 p-3">
            <p class="text-xs text-white/50">学生数</p>
            <p class="text-lg font-semibold text-white">{{ cls.student_count }}</p>
          </div>
        </div>
        
        <div class="mt-4 flex gap-2">
          <Button variant="outline" size="sm" class="flex-1" @click="openViewDialog(cls.name)">
    ...
    <div v-else class="flex h-64 flex-col items-center justify-center text-white/60">
      ...
      <p>暂无班级数据</p>
      <p class="mt-1 text-sm text-white/40">请先在学生管理中添加学生并分配班级</p>
    </div>

    <!-- View Students Dialog -->
    <Dialog v-model:open="showViewDialog" :title="`${viewingClass} - 学生列表`" description="查看班级学生详细信息">
      <div v-if="isLoadingStudents" class="flex h-32 items-center justify-center">
        <Loader2 class="h-6 w-6 animate-spin text-primary" />
      </div>
      
      <div v-else-if="classStudents && classStudents.length > 0" class="max-h-[60vh] overflow-y-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-white/5">
            <tr v-for="student in classStudents" :key="student.id" class="text-white/80">
              <td class="px-4 py-3 font-mono text-xs">{{ student.student_id }}</td>
      ...
      <div v-else class="flex h-32 flex-col items-center justify-center text-white/50">
        <p>该班级暂无学生</p>
      </div>
