...
import { groupsApi, type TeacherScoreRequest } from '@/api'
...
export function useDeleteGroupTask() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ taskId }: { taskId: number }) => groupsApi.deleteTask(taskId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['group-tasks'] })
    },
  })
...
export function useSubmitTeacherScore() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ taskId, data }: { taskId: number; data: TeacherScoreRequest }) =>
